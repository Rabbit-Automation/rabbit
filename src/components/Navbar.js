import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { Button } from './Button';
import './Navbar.css';

function Navbar() {
  const [click, setClick] = useState(false);
  const [button, setButton] = useState(true);

  const handleClick = () => setClick(!click); 
  const closeMobileMenu = () => setClick(false);

  const showButton = () => {
    if(window.innerWidth <= 1100){
      setButton(false);
    } else {
      setButton(true);
    }
  };

  useEffect(() => {
     showButton();
  }, [])
  window.addEventListener('resize', showButton);

  return (
    <>
    <nav className='navbar'>
      <div className='navbar-container'>
        <Link to='/' className='navbar-logo' onClick={closeMobileMenu}>
          <img src='/images/general/logo_b.png' alt='Rabbit Automation and Technologies logo' width='70'/>
        </Link>
        <div className='menu-icon' onClick={handleClick}>
          <i className={ click ? 'fas fa-times' : 'fas fa-bars'}/>
        </div>
        <ul className={ click ? 'nav-menu active' : 'nav-menu' }>
          <li className="nav-item">
            <Link to='/' className='nav-links' onClick={ closeMobileMenu }>
              RABBIT
            </Link>
          </li>
          <li className="nav-item">
            <Link to='#venta_productos' className='nav-links' onClick={ closeMobileMenu }>
              VENTA DE MATERIAL
            </Link>
          </li>
          <li className="nav-item">
            <Link to='/demos' className='nav-links' onClick={ closeMobileMenu }>
              SOLUCIONES
            </Link>
          </li>
          <li className="nav-item">
            <Link to='/contacto' className='nav-links' onClick={ closeMobileMenu }>
              CONTACTO
            </Link>
          </li>
        </ul>
        {/* {button && <Button buttonStyle='btn--outline'>SOLUCIONES</Button>} */}
      </div>
    </nav>
    </>
  )
}

export default Navbar;
